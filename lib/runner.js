        ...
        return this._runSuiteStates(suite, browser)
            .fin(function() {
                if (suite.children.length === 0) {
                    ...
                    _this.on(RunnerEvents.END_SUITE, function() {
                        if (suite.completeInBrowsers[browser.id]) {
                            return;
                        }

                        for (var c in suite.children) {
                            if (!suite.children[c].completeInBrowsers[browser.id]) {
                                return;
                            }
                        }
