...
module.exports = inherit(EventEmitter, {
                ...
                return _this.browserPool.getBrowser(browserId)
                    .then(function(browser) {
                        return _this._runSuiteInBrowser(suite, browser)
                            .fail(function(e) {
                                e.browserSessionId = browser.sessionId;
                                return q.reject(e);
                            })
                            .fin(function() {
        ...
        return this._runSuiteStates(suite, browser)
            .fin(function() {
                if (suite.children.length === 0) {
                    suite.completeInBrowsers[browser.id] = true;
                    _this.emit(RunnerEvents.END_SUITE, eventData);
                } else {
                    if (!_this.awaiting[browser.id]) {
                        _this.awaiting[browser.id] = [];
                    }
                    _this.awaiting[browser.id].push(suite);
                }

                _this._processAwaitingSuits(browser.id);
            });
    },
    ...
    _processAwaitingSuits: function(browser) {
        ...
        suits.forEach(function(suite, i) {
            if (_.every(suite.children, 'completeInBrowsers["' + browser + '"]')) {
                suite.completeInBrowsers[browser] = true;
    ...
    _runStateInSession: function(state, session) {
        ...
        return session.capture(state, {coverage: this.config.coverageEnabled})
            ...
            .fail(function(e) {
                e.suite = suite;
                e.state = state;
                e.browserId = session.browser.id;
